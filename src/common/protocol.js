...
const fallback = normalizeLowerString(source.kind) === 'audit'
  ? auditorDefaults
  : executorDefaults;
...
return {
  ...
  xmlRootTag: normalizeOptionalString(
    source.xmlRootTag,
    fallback.xmlRootTag || (PARSER.XML && PARSER.XML.FILE_ROOT_TAG ? PARSER.XML.FILE_ROOT_TAG : 'File')
  ),
  pathAttribute: normalizeOptionalString(
    source.pathAttribute,
    fallback.pathAttribute || (PARSER.XML && PARSER.XML.FILE_PATH_ATTRIBUTE ? PARSER.XML.FILE_PATH_ATTRIBUTE : 'path')
  ),
...
return {
  ...
  parserErrors: Array.isArray(source.parserErrors)
    ? source.parserErrors.map(function mapEntry(entry) {
...
return {
  ...
  criteria: normalizeArray(source.criteria).map(function mapCriterion(entry) {
    return normalizeOptionalString(entry, '');
  }).filter(Boolean),
  targetFile: normalizeOptionalString(source.targetFile, ''),
  metadata: isPlainObject(source.metadata) ? cloneValue(source.metadata) : createNullObject()
...
return {
  ...
  findings: Array.isArray(source.findings)
    ? source.findings.map(normalizeReviewFinding).filter(function filterFinding(finding) {
...
if (normalizedType === MANUAL_HUB.PACKET_TYPES.TASK_DISPATCH) {
  return normalizeTaskDispatchPayload(payload);
}

if (normalizedType === MANUAL_HUB.PACKET_TYPES.EXECUTION_RESULT) {
  return normalizeExecutionResultPayload(payload);
}

if (normalizedType === MANUAL_HUB.PACKET_TYPES.AUDIT_REQUEST) {
  return normalizeAuditRequestPayload(payload);
}

if (normalizedType === MANUAL_HUB.PACKET_TYPES.AUDIT_RESULT) {
  return normalizeAuditResultPayload(payload);
}
...
return oneOf(normalizeOptionalString(packetType, ''), SUPPORTED_PACKET_TYPES, '');
}

function normalizeMessageType(messageType) {
return oneOf(normalizeOptionalString(messageType, ''), SUPPORTED_MESSAGE_TYPES, '');
}

function normalizeResponseStatus(status) {
return oneOf(normalizeLowerString(status, ''), SUPPORTED_RESPONSE_STATUS_VALUES, MESSAGE_STATUS_OK);
}

function normalizeWorkflowStage(stage) {
return oneOf(normalizeLowerString(stage, ''), SUPPORTED_STAGE_VALUES, DEFAULTS.workflow && DEFAULTS.workflow.stage ? DEFAULTS.workflow.stage : WORKFLOW_STAGES.IDLE || 'idle');
}

function normalizeWorkflowStatus(status) {
return oneOf(normalizeLowerString(status, ''), SUPPORTED_STATUS_VALUES, DEFAULTS.workflow && DEFAULTS.workflow.status ? DEFAULTS.workflow.status : WORKFLOW_STATUSES.IDLE || 'idle');
}

function normalizeWorkflowState(value) {
const source = isPlainObject(value) ? value : createNullObject();
const defaults = isPlainObject(DEFAULTS.workflow) ? DEFAULTS.workflow : createNullObject();
const selectedProviderIds = isPlainObject(source.selectedProviderIds)
? source.selectedProviderIds
: (isPlainObject(defaults.selectedProviderIds) ? defaults.selectedProviderIds : createNullObject());
...
return {
  ...
  activeProviderId: oneOf(
    normalizeLowerString(hasOwn(source, 'activeProviderId') ? source.activeProviderId : defaults.activeProviderId, ''),
    SUPPORTED_PROVIDER_IDS.concat(['']),
    ''
  ),
  selectedProviderIds: {
    designer: oneOf(
      normalizeLowerString(selectedProviderIds.designer, ''),
      SUPPORTED_PROVIDER_IDS.concat(['']),
      defaults.selectedProviderIds && defaults.selectedProviderIds.designer ? defaults.selectedProviderIds.designer : ''
    ),
    executor: oneOf(
      normalizeLowerString(selectedProviderIds.executor, ''),
      SUPPORTED_PROVIDER_IDS.concat(['']),
      defaults.selectedProviderIds && defaults.selectedProviderIds.executor ? defaults.selectedProviderIds.executor : ''
    ),
    auditor: oneOf(
      normalizeLowerString(selectedProviderIds.auditor, ''),
      SUPPORTED_PROVIDER_IDS.concat(['']),
      defaults.selectedProviderIds && defaults.selectedProviderIds.auditor ? defaults.selectedProviderIds.auditor : ''
    )
  },
  ...
  latestExecutorResponse: normalizeOptionalString(
    hasOwn(source, 'latestExecutorResponse') ? source.latestExecutorResponse : defaults.latestExecutorResponse,
    ''
  ),
  latestAuditVerdict: oneOf(
    normalizeOptionalString(hasOwn(source, 'latestAuditVerdict') ? source.latestAuditVerdict : defaults.latestAuditVerdict, ''),
    SUPPORTED_REVIEW_VERDICTS.concat(['']),
    ''
  ),
  latestAuditSummary: normalizeOptionalString(
    hasOwn(source, 'latestAuditSummary') ? source.latestAuditSummary : defaults.latestAuditSummary,
    ''
  ),
...
errors.push({
  ...
  details: {
    value: value,
    allowedValues: cloneValue(allowedValues)
  }
});
...
if (issue.number === null || !Number.isFinite(Number(issue.number))) {
  errors.push({
    field: prefix + '.number',
    code: ERROR_CODES.INVALID_ARGUMENT || 'INVALID_ARGUMENT',
    message: prefix + '.number must be a finite integer.'
  });
}
...
if (outputContract.singleFile !== true) {
  errors.push({
    field: prefix + '.singleFile',
    code: ERROR_CODES.INVALID_ARGUMENT || 'INVALID_ARGUMENT',
    message: prefix + '.singleFile must be true for this protocol version.'
  });
}
...
if (!hasOwn(normalized, 'parsedOutput')) {
  errors.push({
    field: 'payload.parsedOutput',
    code: ERROR_CODES.INVALID_ARGUMENT || 'INVALID_ARGUMENT',
    message: 'payload.parsedOutput must exist.'
  });
}
...
if (!Array.isArray(normalized.criteria) || normalized.criteria.length === 0) {
  errors.push({
    field: 'payload.criteria',
    code: ERROR_CODES.INVALID_ARGUMENT || 'INVALID_ARGUMENT',
    message: 'payload.criteria must contain at least one review criterion.'
  });
}
...
if (packetType === MANUAL_HUB.PACKET_TYPES.TASK_DISPATCH) {
  result = validateTaskDispatchPayload(payload, errors);
} else if (packetType === MANUAL_HUB.PACKET_TYPES.EXECUTION_RESULT) {
  result = validateExecutionResultPayload(payload, errors);
} else if (packetType === MANUAL_HUB.PACKET_TYPES.AUDIT_REQUEST) {
  result = validateAuditRequestPayload(payload, errors);
} else if (packetType === MANUAL_HUB.PACKET_TYPES.AUDIT_RESULT) {
  result = validateAuditResultPayload(payload, errors);
} else {
  errors.push({
    field: 'packetType',
    code: ERROR_CODES.INVALID_ARGUMENT || 'INVALID_ARGUMENT',
    message: 'Unsupported packet type: ' + String(packetType)
  });
  result = {
    valid: false,
    normalized: cloneOrPrimitive(payload)
  };
}

return {
  valid: errors.length === 0 && result.valid === true,
...
assert(type, ERROR_CODES.MESSAGE_UNSUPPORTED || 'MESSAGE_UNSUPPORTED', 'Unsupported message type.', {
  type: source.type,
  supportedTypes: cloneValue(SUPPORTED_MESSAGE_TYPES)
});
...
return createMessage({
  ...
  meta: Object.assign(createNullObject(), toSafeObject(request.meta), toSafeObject(config.meta), {
    isResponse: true
  })
});
...
function createOkResponse(requestMessage, data, options) {
return createResponse(requestMessage, MESSAGE_STATUS_OK, data, options);
}

function createErrorResponse(requestMessage, error, options) {
return createResponse(requestMessage, MESSAGE_STATUS_ERROR, normalizeErrorPayload(error), options);
}

function isResponseMessage(message) {
return isPlainObject(message)
&& isPlainObject(message.payload)
&& hasOwn(message.payload, 'status')
&& normalizeResponseStatus(message.payload.status) === normalizeLowerString(message.payload.status, '');
}
...
if (!source) {
  return {
    ...
    errors: [{
      field: 'message',
      code: ERROR_CODES.INVALID_ARGUMENT || 'INVALID_ARGUMENT',
      message: 'Message must be a plain object.'
    }]
  };
}
...
if (!isPlainObject(normalized.target)) {
  errors.push({
    field: 'target',
    code: ERROR_CODES.INVALID_ARGUMENT || 'INVALID_ARGUMENT',
...
assert(packetType, ERROR_CODES.INVALID_ARGUMENT || 'INVALID_ARGUMENT', 'Unsupported packet type.', {
  packetType: source.packetType,
  supportedPacketTypes: cloneValue(SUPPORTED_PACKET_TYPES)
});
...
if (!source) {
  return {
    ...
    errors: [{
      field: 'packet',
      code: ERROR_CODES.INVALID_ARGUMENT || 'INVALID_ARGUMENT',
      message: 'Packet must be a plain object.'
    }]
  };
}
...
if (config.expectedPacketType) {
  ...
  if (expectedPacketType && normalized.packetType !== expectedPacketType) {
    errors.push({
      field: 'packetType',
      code: ERROR_CODES.INVALID_ARGUMENT || 'INVALID_ARGUMENT',
      message: 'Packet type mismatch.',
      details: {
        expectedPacketType: expectedPacketType,
        actualPacketType: normalized.packetType
      }
    });
  }
}
...
const beginDelimiter = normalizeOptionalString(
config.beginDelimiter,
MANUAL_HUB.DELIMITERS && MANUAL_HUB.DELIMITERS.BEGIN ? MANUAL_HUB.DELIMITERS.BEGIN : '[MAOE_PACKET_BEGIN]'
);
const endDelimiter = normalizeOptionalString(
config.endDelimiter,
MANUAL_HUB.DELIMITERS && MANUAL_HUB.DELIMITERS.END ? MANUAL_HUB.DELIMITERS.END : '[MAOE_PACKET_END]'
);
const fenceLanguage = normalizeOptionalString(
config.fenceLanguage,
MANUAL_HUB.CLIPBOARD && MANUAL_HUB.CLIPBOARD.PREFERRED_FENCE_LANGUAGE
...
const fencedMatch = innerText.match(/```(?:json|text|txt)?\s*([\s\S]*?)```/i);
...
function parsePacketText(rawText, options) {
const config = isPlainObject(options) ? options : createNullObject();
const sourceText = typeof rawText === 'string' ? rawText : '';
const extracted = extractDelimitedPacketText(sourceText, config) || sourceText.trim();
...
if (!extracted) {
  return {
    ...
    errors: [{
      field: 'rawText',
      code: ERROR_CODES.PARSER_INVALID_JSON || 'PARSER_INVALID_JSON',
      message: 'Packet text is empty.'
    }]
  };
}
...
if (!isPlainObject(parsed)) {
  return {
    ...
    errors: [{
      field: 'rawText',
      code: ERROR_CODES.PARSER_INVALID_JSON || 'PARSER_INVALID_JSON',
      message: 'Packet text is not valid JSON.'
    }]
  };
}

return validatePacket(parsed, config);

}

function createTaskDispatchPacket(payload, options) {
const config = isPlainObject(options) ? options : createNullObject();

return createPacket({
  packetType: MANUAL_HUB.PACKET_TYPES.TASK_DISPATCH,
  requestId: config.requestId,
  createdAt: config.createdAt,
  source: config.source,
  target: config.target,
...
function createExecutionResultPacket(payload, options) {
const config = isPlainObject(options) ? options : createNullObject();

return createPacket({
  packetType: MANUAL_HUB.PACKET_TYPES.EXECUTION_RESULT,
  requestId: config.requestId,
  createdAt: config.createdAt,
  source: config.source,
  target: config.target,
...
function createAuditRequestPacket(payload, options) {
const config = isPlainObject(options) ? options : createNullObject();

return createPacket({
  packetType: MANUAL_HUB.PACKET_TYPES.AUDIT_REQUEST,
  requestId: config.requestId,
  createdAt: config.createdAt,
  source: config.source,
  target: config.target,
...
function createAuditResultPacket(payload, options) {
const config = isPlainObject(options) ? options : createNullObject();

return createPacket({
  packetType: MANUAL_HUB.PACKET_TYPES.AUDIT_RESULT,
  requestId: config.requestId,
  createdAt: config.createdAt,
  source: config.source,
  target: config.target,
...
const normalizedFrom = normalizeWorkflowStage(fromStage);
const normalizedTo = normalizeWorkflowStage(toStage);
const allowed = hasOwn(STAGE_TRANSITIONS, normalizedFrom)
? STAGE_TRANSITIONS[normalizedFrom]
: [];

return allowed.indexOf(normalizedTo) >= 0;

}

function canTransitionStatus(fromStatus, toStatus) {
const normalizedFrom = normalizeWorkflowStatus(fromStatus);
const normalizedTo = normalizeWorkflowStatus(toStatus);
const allowed = hasOwn(STATUS_TRANSITIONS, normalizedFrom)
? STATUS_TRANSITIONS[normalizedFrom]
: [];
...
validateOneOf(normalized.stage, SUPPORTED_STAGE_VALUES, 'stage', errors);
validateOneOf(normalized.status, SUPPORTED_STATUS_VALUES, 'status', errors);

if (normalized.latestAuditVerdict) {
  validateOneOf(normalized.latestAuditVerdict, SUPPORTED_REVIEW_VERDICTS, 'latestAuditVerdict', errors);
}

if (normalized.activeProviderId) {
  validateOneOf(normalized.activeProviderId, SUPPORTED_PROVIDER_IDS, 'activeProviderId', errors);
}

if (normalized.selectedProviderIds.designer) {
  validateOneOf(normalized.selectedProviderIds.designer, SUPPORTED_PROVIDER_IDS, 'selectedProviderIds.designer', errors);
}

if (normalized.selectedProviderIds.executor) {
  validateOneOf(normalized.selectedProviderIds.executor, SUPPORTED_PROVIDER_IDS, 'selectedProviderIds.executor', errors);
}

if (normalized.selectedProviderIds.auditor) {
  validateOneOf(normalized.selectedProviderIds.auditor, SUPPORTED_PROVIDER_IDS, 'selectedProviderIds.auditor', errors);
}

if (normalized.pullRequestNumber !== null && !Number.isFinite(Number(normalized.pullRequestNumber))) {
  errors.push({
    field: 'pullRequestNumber',
    code: ERROR_CODES.INVALID_ARGUMENT || 'INVALID_ARGUMENT',
    message: 'pullRequestNumber must be null or a finite integer.'
  });
}

if (normalized.currentIssueNumber !== null && !Number.isFinite(Number(normalized.currentIssueNumber))) {
  errors.push({
    field: 'currentIssueNumber',
    code: ERROR_CODES.INVALID_ARGUMENT || 'INVALID_ARGUMENT',
    message: 'currentIssueNumber must be null or a finite integer.'
  });
}
...
function assertValidWorkflowState(value) {
const validation = validateWorkflowState(value);
...
function transitionWorkflowState(currentState, patch, options) {
const config = isPlainObject(options) ? options : createNullObject();
const current = assertValidWorkflowState(currentState);
const incomingPatch = isPlainObject(patch) ? patch : createNullObject();

const candidate = normalizeWorkflowState(Object.assign(createNullObject(), current, incomingPatch, {
  selectedProviderIds: Object.assign(
    createNullObject(),
    current.selectedProviderIds,
    isPlainObject(incomingPatch.selectedProviderIds) ? incomingPatch.selectedProviderIds : createNullObject()
  )
}));

const fromStage = current.stage;
const toStage = candidate.stage;
const fromStatus = current.status;
const toStatus = candidate.status;

if (config.skipTransitionCheck !== true) {
  assert(
    canTransitionStage(fromStage, toStage),
    ERROR_CODES.INVALID_STATE || 'INVALID_STATE',
    'Invalid workflow stage transition.',
    {
      fromStage: fromStage,
      toStage: toStage,
      allowed: cloneValue(STAGE_TRANSITIONS[fromStage] || [])
    }
  );
  ...
  assert(
    ...
    'Invalid workflow status transition.',
    {
      ...
      toStatus: toStatus,
      allowed: cloneValue(STATUS_TRANSITIONS[fromStatus] || [])
    }
  );
}

candidate.lastTransitionAt = normalizeOptionalString(incomingPatch.lastTransitionAt, nowIsoString());

if (config.humanAction) {
  const humanAction = normalizeLowerString(config.humanAction, '');

  if (Object.keys(HUMAN_ACTIONS).map(function mapActionKey(key) {
    return HUMAN_ACTIONS[key];
  }).indexOf(humanAction) >= 0) {
    candidate.lastHumanActionAt = nowIsoString();
  }
}
...
function createHumanActionRecord(action, details) {
const normalizedAction = normalizeLowerString(action, '');

assert(
  Object.keys(HUMAN_ACTIONS).map(function mapActionKey(key) {
    return HUMAN_ACTIONS[key];
  }).indexOf(normalizedAction) >= 0,
  ERROR_CODES.INVALID_ARGUMENT || 'INVALID_ARGUMENT',
  'Unsupported human action.',
  {
    action: action,
    supportedActions: Object.keys(HUMAN_ACTIONS).map(function mapActionKey(key) {
      return HUMAN_ACTIONS[key];
    })
  }
);
...
function createReviewDecision(value) {
const source = isPlainObject(value) ? value : createNullObject();
const verdict = oneOf(source.verdict, SUPPORTED_REVIEW_VERDICTS, '');
...
assert(
  ...
  'Invalid review verdict.',
  {
    ...
    supportedVerdicts: cloneValue(SUPPORTED_REVIEW_VERDICTS)
  }
);
...
return deepFreeze({
  ...
  findings: Array.isArray(source.findings)
    ...
    : [],
  at: normalizeOptionalString(source.at, nowIsoString())
});

}

function createRepositoryTreeSnapshot(value) {
const source = isPlainObject(value) ? value : createNullObject();
...
return deepFreeze({
  ...
  repo: normalizeOptionalString(source.repo, ''),
  baseBranch: normalizeOptionalString(source.baseBranch, ''),
  treeSha: normalizeOptionalString(source.treeSha, ''),
  generatedAt: normalizeOptionalString(source.generatedAt, nowIsoString()),
  paths: Array.isArray(source.paths)
    ? uniqueStrings(source.paths)
    : [],
  rawTreeText: normalizeOptionalString(source.rawTreeText, '')
});

}

function createTaskContext(value) {
const source = isPlainObject(value) ? value : createNullObject();
const issue = normalizeIssueRef(source.issue);
const repository = normalizeGitHubRepositoryRef(source.repository);
const targetFile = normalizeOptionalString(source.targetFile, '');
const issueNumber = issue.number;
const issueTitle = issue.title;
const workingBranchPrefix = normalizeOptionalString(source.workingBranchPrefix, '');

return deepFreeze({
  repository: repository,
  issue: issue,
  targetFile: targetFile,
  workingBranch: typeof helpers.buildBranchName === 'function'
    ? helpers.buildBranchName(issueNumber, issueTitle, workingBranchPrefix)
    : '',
  createdAt: normalizeOptionalString(source.createdAt, nowIsoString())
});
...
function createExecutorInstruction(value) {
const source = isPlainObject(value) ? value : createNullObject();
...
function createAuditorInstruction(value) {
const source = isPlainObject(value) ? value : createNullObject();
...
return deepFreeze({
  ...
  outputContract: normalizeOutputContract(Object.assign(createNullObject(), source.outputContract, {
    kind: 'audit'
  }))
});
...
function normalizeManualBridgeDraft(value) {
const source = isPlainObject(value) ? value : createNullObject();

return {
  lastPacketType: normalizePacketType(source.lastPacketType),
  lastPacketText: normalizeOptionalString(source.lastPacketText, ''),
  lastResponseText: normalizeOptionalString(source.lastResponseText, ''),
  clipboardFormat: normalizeOptionalString(source.clipboardFormat, MANUAL_HUB.CLIPBOARD && MANUAL_HUB.CLIPBOARD.PREFERRED_FENCE_LANGUAGE
    ? MANUAL_HUB.CLIPBOARD.PREFERRED_FENCE_LANGUAGE
    : 'json')
};
...
function createStateChangedMessage(currentState, options) {
const config = isPlainObject(options) ? options : createNullObject();
const normalizedState = assertValidWorkflowState(currentState);

return createMessage({
  type: MESSAGE_TYPES.BACKGROUND_STATE_CHANGED,
  requestId: config.requestId,
  createdAt: config.createdAt,
  source: config.source,
  target: config.target,
  payload: {
    workflow: normalizedState
  },
  meta: config.meta
});
...
function createBackgroundErrorMessage(error, options) {
const config = isPlainObject(options) ? options : createNullObject();

return createMessage({
  type: MESSAGE_TYPES.BACKGROUND_ERROR,
  requestId: config.requestId,
  createdAt: config.createdAt,
  source: config.source,
  target: config.target,
...
function createLogAppendedMessage(logEntry, options) {
const config = isPlainObject(options) ? options : createNullObject();

return createMessage({
  type: MESSAGE_TYPES.BACKGROUND_LOG_APPENDED,
  requestId: config.requestId,
  createdAt: config.createdAt,
  source: config.source,
  target: config.target,
  payload: {
    entry: cloneOrPrimitive(logEntry)
  },
  meta: config.meta
});
...
function createContentProbeMessage(siteInfo, options) {
const config = isPlainObject(options) ? options : createNullObject();
...
return createMessage({
  ...
  source: config.source,
  target: config.target,
  payload: isPlainObject(siteInfo) ? cloneValue(siteInfo) : createNullObject(),
  meta: config.meta
});

}

function createFillPromptMessage(promptText, options) {
const config = isPlainObject(options) ? options : createNullObject();

return createMessage({
  type: MESSAGE_TYPES.CONTENT_FILL_PROMPT,
  requestId: config.requestId,
  createdAt: config.createdAt,
  source: config.source,
  target: config.target,
  payload: {
    promptText: normalizeOptionalString(promptText, ''),
    autoSubmit: normalizeBoolean(config.autoSubmit, false)
  },
  meta: config.meta
});
...
function createExtractLatestResponseMessage(options) {
const config = isPlainObject(options) ? options : createNullObject();

return createMessage({
  type: MESSAGE_TYPES.CONTENT_EXTRACT_LATEST_RESPONSE,
  requestId: config.requestId,
  createdAt: config.createdAt,
  source: config.source,
  target: config.target,
  payload: {
    extractionStrategy: normalizeOptionalString(config.extractionStrategy, 'fenced_blocks')
  },
  meta: config.meta
});
...
function createBuildManualPacketMessage(packet, options) {
const config = isPlainObject(options) ? options : createNullObject();
const normalizedPacket = assertValidPacket(packet);

return createMessage({
  type: MESSAGE_TYPES.CONTENT_BUILD_MANUAL_PACKET,
  requestId: config.requestId,
  createdAt: config.createdAt,
  source: config.source,
  target: config.target,
  payload: {
    ...
    packetText: buildPacketEnvelopeText(normalizedPacket, config.packetFormat)
  },
  meta: config.meta
});
...
function createSiteDetectedMessage(siteInfo, options) {
const config = isPlainObject(options) ? options : createNullObject();

return createMessage({
  type: MESSAGE_TYPES.CONTENT_SITE_DETECTED,
  requestId: config.requestId,
  createdAt: config.createdAt,
  source: config.source,
  target: config.target,
...
function createAiOutputCapturedMessage(capturedOutput, options) {
const config = isPlainObject(options) ? options : createNullObject();

return createMessage({
  type: MESSAGE_TYPES.CONTENT_AI_OUTPUT_CAPTURED,
  requestId: config.requestId,
  createdAt: config.createdAt,
  source: config.source,
  target: config.target,
  payload: {
    rawText: normalizeOptionalString(capturedOutput && capturedOutput.rawText, ''),
    extractedBlocks: Array.isArray(capturedOutput && capturedOutput.extractedBlocks)
      ? cloneValue(capturedOutput.extractedBlocks)
      : [],
    parsedPacket: capturedOutput && hasOwn(capturedOutput, 'parsedPacket')
      ? cloneOrPrimitive(capturedOutput.parsedPacket)
      : null
  },
  meta: config.meta
});
...
const MESSAGE_SCHEMAS = deepFreeze({
[MESSAGE_TYPES.POPUP_GET_BOOTSTRAP]: {
type: MESSAGE_TYPES.POPUP_GET_BOOTSTRAP,
payloadKind: 'object'
},
[MESSAGE_TYPES.POPUP_SAVE_GITHUB_SETTINGS]: {
type: MESSAGE_TYPES.POPUP_SAVE_GITHUB_SETTINGS,
payloadKind: 'object'
},
[MESSAGE_TYPES.POPUP_LOAD_ISSUES]: {
type: MESSAGE_TYPES.POPUP_LOAD_ISSUES,
payloadKind: 'object'
},
[MESSAGE_TYPES.POPUP_LOAD_REPO_TREE]: {
type: MESSAGE_TYPES.POPUP_LOAD_REPO_TREE,
payloadKind: 'object'
},
[MESSAGE_TYPES.POPUP_SELECT_ISSUE]: {
type: MESSAGE_TYPES.POPUP_SELECT_ISSUE,
payloadKind: 'object'
},
[MESSAGE_TYPES.POPUP_SUBMIT_HUMAN_PAYLOAD]: {
type: MESSAGE_TYPES.POPUP_SUBMIT_HUMAN_PAYLOAD,
payloadKind: 'object'
},
[MESSAGE_TYPES.POPUP_ADVANCE_STAGE]: {
type: MESSAGE_TYPES.POPUP_ADVANCE_STAGE,
payloadKind: 'object'
},
[MESSAGE_TYPES.POPUP_RESET_WORKFLOW]: {
type: MESSAGE_TYPES.POPUP_RESET_WORKFLOW,
payloadKind: 'object'
},
[MESSAGE_TYPES.POPUP_GET_WORKFLOW_STATE]: {
type: MESSAGE_TYPES.POPUP_GET_WORKFLOW_STATE,
payloadKind: 'object'
},
[MESSAGE_TYPES.POPUP_GET_EVENT_LOG]: {
type: MESSAGE_TYPES.POPUP_GET_EVENT_LOG,
payloadKind: 'object'
},
...
[MESSAGE_TYPES.CONTENT_PROBE]: {
type: MESSAGE_TYPES.CONTENT_PROBE,
payloadKind: 'object'
},
[MESSAGE_TYPES.CONTENT_FILL_PROMPT]: {
type: MESSAGE_TYPES.CONTENT_FILL_PROMPT,
payloadKind: 'object'
},
[MESSAGE_TYPES.CONTENT_EXTRACT_LATEST_RESPONSE]: {
type: MESSAGE_TYPES.CONTENT_EXTRACT_LATEST_RESPONSE,
payloadKind: 'object'
},
[MESSAGE_TYPES.CONTENT_BUILD_MANUAL_PACKET]: {
type: MESSAGE_TYPES.CONTENT_BUILD_MANUAL_PACKET,
payloadKind: 'object'
},
[MESSAGE_TYPES.CONTENT_SITE_DETECTED]: {
type: MESSAGE_TYPES.CONTENT_SITE_DETECTED,
payloadKind: 'object'
},
[MESSAGE_TYPES.CONTENT_AI_OUTPUT_CAPTURED]: {
type: MESSAGE_TYPES.CONTENT_AI_OUTPUT_CAPTURED,
payloadKind: 'object'
}
});

const PACKET_SCHEMAS = deepFreeze({
[MANUAL_HUB.PACKET_TYPES.TASK_DISPATCH]: {
packetType: MANUAL_HUB.PACKET_TYPES.TASK_DISPATCH,
requiredFields: cloneValue(PACKET_REQUIRED_FIELDS.TASK_DISPATCH || [])
},
[MANUAL_HUB.PACKET_TYPES.EXECUTION_RESULT]: {
packetType: MANUAL_HUB.PACKET_TYPES.EXECUTION_RESULT,
requiredFields: cloneValue(PACKET_REQUIRED_FIELDS.EXECUTION_RESULT || [])
},
[MANUAL_HUB.PACKET_TYPES.AUDIT_REQUEST]: {
packetType: MANUAL_HUB.PACKET_TYPES.AUDIT_REQUEST,
requiredFields: cloneValue(PACKET_REQUIRED_FIELDS.AUDIT_REQUEST || [])
},
[MANUAL_HUB.PACKET_TYPES.AUDIT_RESULT]: {
packetType: MANUAL_HUB.PACKET_TYPES.AUDIT_RESULT,
requiredFields: cloneValue(PACKET_REQUIRED_FIELDS.AUDIT_RESULT || [])
}
});

const api = {
version: PROTOCOL_VERSION,
messageTypes: deepFreeze(cloneValue(MESSAGE_TYPES)),
packetTypes: deepFreeze(cloneValue(MANUAL_HUB.PACKET_TYPES || createNullObject())),
reviewVerdicts: deepFreeze(cloneValue(REVIEW_VERDICTS)),
workflowStages: deepFreeze(cloneValue(WORKFLOW_STAGES)),
workflowStatuses: deepFreeze(cloneValue(WORKFLOW_STATUSES)),
stageTransitions: deepFreeze(cloneValue(STAGE_TRANSITIONS)),
statusTransitions: deepFreeze(cloneValue(STATUS_TRANSITIONS)),
messageSchemas: MESSAGE_SCHEMAS,
packetSchemas: PACKET_SCHEMAS,
...
assertValidPacket: assertValidPacket,
parsePacketText: parsePacketText,
buildPacketEnvelopeText: buildPacketEnvelopeText,
extractDelimitedPacketText: extractDelimitedPacketText,
createTaskDispatchPacket: createTaskDispatchPacket,
createExecutionResultPacket: createExecutionResultPacket,
createAuditRequestPacket: createAuditRequestPacket,
createAuditResultPacket: createAuditResultPacket,
normalizeTaskDispatchPayload: normalizeTaskDispatchPayload,
normalizeExecutionResultPayload: normalizeExecutionResultPayload,
normalizeAuditRequestPayload: normalizeAuditRequestPayload,
normalizeAuditResultPayload: normalizeAuditResultPayload,
normalizeWorkflowState: normalizeWorkflowState,
validateWorkflowState: validateWorkflowState,
assertValidWorkflowState: assertValidWorkflowState,
...
createHumanActionRecord: createHumanActionRecord,
createReviewDecision: createReviewDecision,
createRepositoryTreeSnapshot: createRepositoryTreeSnapshot,
createTaskContext: createTaskContext,
createExecutorInstruction: createExecutorInstruction,
createAuditorInstruction: createAuditorInstruction,
normalizeManualBridgeDraft: normalizeManualBridgeDraft,
createStateChangedMessage: createStateChangedMessage,
createBackgroundErrorMessage: createBackgroundErrorMessage,
createLogAppendedMessage: createLogAppendedMessage,
createContentProbeMessage: createContentProbeMessage,
createFillPromptMessage: createFillPromptMessage,
createExtractLatestResponseMessage: createExtractLatestResponseMessage,
createBuildManualPacketMessage: createBuildManualPacketMessage,
createSiteDetectedMessage: createSiteDetectedMessage,
createAiOutputCapturedMessage: createAiOutputCapturedMessage,
helpers: deepFreeze({
nowIsoString: nowIsoString,
generateRequestId: generateRequestId,
createProtocolError: createProtocolError,
normalizeSourceTargetEndpoint: normalizeSourceTargetEndpoint,
normalizeErrorPayload: normalizeErrorPayload,
normalizeMessageType: normalizeMessageType,
normalizePacketType: normalizePacketType,
normalizeWorkflowStage: normalizeWorkflowStage,
normalizeWorkflowStatus: normalizeWorkflowStatus,
validatePacketPayload: validatePacketPayload,
toSafeObject: toSafeObject,
stringMapFromObject: stringMapFromObject
})
};

try {
logger.debug('Protocol module registered.', {
protocolVersion: PROTOCOL_VERSION,
messageTypeCount: SUPPORTED_MESSAGE_TYPES.length,
packetTypeCount: SUPPORTED_PACKET_TYPES.length
});
} catch (error) {
}

root.registerValue('protocol', deepFreeze(api), {
overwrite: false,
freeze: false,
clone: false
});
